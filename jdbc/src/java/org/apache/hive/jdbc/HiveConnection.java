...
import org.apache.hadoop.hive.kerberos.HiveKerbTransport;
...
public class HiveConnection implements java.sql.Connection {
        ...
        try {
            ...
            if (!HIVE_AUTH_SIMPLE.equals(sessConfMap.get(HIVE_AUTH_TYPE))) {
                ...
                if (sessConfMap.containsKey(HIVE_AUTH_PRINCIPAL)) {
                     ...
                     * HIVE_AUTH_KERBEROS_AUTH_TYPE_FROM_SUBJECT
                    ...
                    transport = HiveKerbTransport.openKerbTransport(
                            sessConfMap.get(HIVE_AUTH_PRINCIPAL), host,
                            transport, saslProps, userName, passwd);
                } else {
    ...
    private void openSession(Map<String, String> sessVars) throws SQLException {
        try {
            TProtocolVersion[] protocols = {
                    TProtocolVersion.HIVE_CLI_SERVICE_PROTOCOL_V6, // Hive 0.13+
                    TProtocolVersion.HIVE_CLI_SERVICE_PROTOCOL_V5, // CDH5
                    TProtocolVersion.HIVE_CLI_SERVICE_PROTOCOL_V3, // Hive 0.12+
                    TProtocolVersion.HIVE_CLI_SERVICE_PROTOCOL_V1 // Hive
                                                                  // prehistory
            };

            TOpenSessionResp openResp = null;
            for (int i = 0; i < protocols.length; i++) {
                TProtocolVersion protocol = protocols[i];
                ...
                openReq.setClient_protocol(protocol);
                LOG.info("HiveConnection.openSession: trying " + protocol);
                try {
                    openResp = client.OpenSession(openReq);
                    // validate connection
                    Utils.verifySuccess(openResp.getStatus());
                    LOG.info("HiveConnection.openSession: succeeded with "
                            + protocol);
                    break;
                } catch (Exception e) {
                    LOG.info("HiveConnection.openSession: failed with "
                            + protocol);
                    ...
                    openTransport();
                    // set up the client
                    client = new TCLIService.Client(new TBinaryProtocol(
                            transport));
                }
            }
    ...
    private void configureConnection(String dbName) throws SQLException {
        ...
        if (isEmbeddedMode) {
            ...
            for (Entry<String, String> hiveConf : hiveConfMap.entrySet()) {
                stmt.execute("set " + hiveConf.getKey() + "="
                        + hiveConf.getValue());
            }
            ...
            // if the client is setting a non-default db, then switch the
            // database
